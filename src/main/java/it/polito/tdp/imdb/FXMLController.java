...
public class FXMLController {
    ...
    void doCreaGrafo(ActionEvent event) {
    	if(boxAnno.getValue()!=null) {
    		...
    		boxRegista.getItems().addAll(model.getAllDirettoriAnno(anno));
    	}
    	else 
    ...
    void doRegistiAdiacenti(ActionEvent event) {
    	if(boxAnno.getValue()!=null) {
    		...
    		int anno=boxAnno.getValue();
    		txtResult.appendText(model.adiacente(boxRegista.getValue()));
    		}
    		else 
    ...
    void doRicorsione(ActionEvent event) {
    	try {
    		if(!txtAttoriCondivisi.getText().equals("") && !txtAttoriCondivisi.getText().equals(null)) {
    		int c=Integer.parseInt(txtAttoriCondivisi.getText());
    		if(boxRegista.getValue()!=null) {
        		model.init(boxRegista.getValue(), c);
        		txtResult.appendText(model.listRegistiAffini());
        		}
        		else 
            		txtResult.setText("Selezionare un regista dalla tendina");
    	}
    		else txtResult.setText("Il formato di attori condivisi non è stato inserito inserire un numero intero");
    	}
    	catch(NumberFormatException e) {
    		txtResult.setText("Il formato di attori condivisi è errato inserire un numero intero");
    	}
